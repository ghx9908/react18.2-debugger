...
import { suspenseStackCursor, popSuspenseListContext, popSuspenseHandler, pushSuspenseListContext, setShallowSuspenseListContext, ForceSuspenseFallback, setDefaultShallowSuspenseListContext, isBadSuspenseFallback } from './ReactFiberSuspenseContext.new';
...
import { renderDidSuspend, renderDidSuspendDelayIfPossible, renderHasNotSuspendedYet, getRenderTargetTime, getWorkInProgressTransitions } from './ReactFiberWorkLoop.new';
...
import { popMarkerInstance, popRootMarkerInstance } from './ReactFiberTracingMarkerComponent.new';
...
if (supportsMutation) {
  ...
  appendAllChildren = function (parent, workInProgress, needsVisibilityToggle, isHidden) {
    ...
    while (node !== null) {
      ...
      } else if (node.tag === HostPortal || (enableHostSingletons && supportsSingletons ? node.tag === HostSingleton : false)) {
  ...
  updateHostComponent = function (current, workInProgress, type, newProps) {
    ...
    // related to `hidden`.
    const updatePayload = prepareUpdate(instance, type, oldProps, newProps, currentHostContext);
