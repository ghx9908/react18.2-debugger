...
import { createWorkInProgress, assignFiberPropertiesInDEV, resetWorkInProgress } from './ReactFiber.new';
...
import { enqueueConcurrentRenderForLane, finishQueueingConcurrentUpdates, getConcurrentlyUpdatedLanes } from './ReactFiberConcurrentUpdates.new';
import { markNestedUpdateScheduled, recordCommitTime, resetNestedUpdateFlag, startProfilerTimer, stopProfilerTimerIfRunningAndRecordDelta, syncNestedUpdateFlag } from './ReactProfilerTimer.new';
...
import { isLegacyActEnvironment, isConcurrentActEnvironment } from './ReactFiberAct.new';
import { processTransitionCallbacks } from './ReactFiberTracingMarkerComponent.new';
import { SuspenseException, getSuspendedThenable, getThenableStateAfterSuspending, isThenableStateResolved } from './ReactFiberThenable.new';
import { schedulePostPaintCallback } from './ReactPostPaintCallback';
import { getSuspenseHandler, isBadSuspenseFallback } from './ReactFiberSuspenseContext.new';
...
const SuspendedAndReadyToUnwind = 4;
...
let workInProgressSuspendedThenableState = null;
...
function prepareFreshStack(root, lanes) {
  ...
  if (workInProgress !== null) {
    let interruptedWork = workInProgressSuspendedReason === NotSuspended ? workInProgress.return : workInProgress;
      ...
      if (workInProgressSuspendedReason !== NotSuspended && workInProgress !== null) {
        ...
        switch (workInProgressSuspendedReason) {
          ...
          case SuspendedOnData:
            {
              const didResolve = workInProgressSuspendedThenableState !== null && isThenableStateResolved(workInProgressSuspendedThenableState);
              if (didResolve) {
                workInProgressSuspendedReason = NotSuspended;
                workInProgressThrownValue = null;
                ...
                const thenable = workInProgressThrownValue;
                const onResolution = () => {
                  ensureRootIsScheduled(root, now());
                };
...
function replaySuspendedUnitOfWork(unitOfWork, thrownValue) {
...
export function getSuspendedThenableState() {
...
if (__DEV__ && replayFailedUnitOfWorkWithInvokeGuardedCallback) {
  ...
  beginWork = (current, unitOfWork, lanes) => {
    ...
    try {
      ...
      if (didSuspendOrErrorWhileHydratingDEV() || originalError === SuspenseException || originalError !== null && typeof originalError === 'object' && typeof originalError.then === 'function') {
