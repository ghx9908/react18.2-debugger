...
import { enableCreateEventHandleAPI, enableProfilerTimer, enableProfilerCommitHooks, enableProfilerNestedUpdatePhase, enableSchedulingProfiler, enableSuspenseCallback, enableScopeAPI, deletedTreeCleanUpLevel, enableUpdaterTracking, enableCache, enableTransitionTracing, enableUseEventHook, enableFloat, enableLegacyHidden, enableHostSingletons } from 'shared/ReactFeatureFlags';
...
import { TransitionRoot, TransitionTracingMarker } from './ReactFiberTracingMarkerComponent.old';
...
function commitBeforeMutationEffectsOnFiber(finishedWork) {
  ...
  if (enableCreateEventHandleAPI) {
    if (!shouldFireAfterActiveInstanceBlur && focusedInstanceHandle !== null) {
      ...
      if (finishedWork.tag === SuspenseComponent && isSuspenseBoundaryBeingHidden(current, finishedWork) &&
...
function insertOrAppendPlacementNodeIntoContainer(node, before, parent) {
  ...
  } else if (tag === HostPortal || (enableHostSingletons && supportsSingletons ? tag === HostSingleton : false)) {
...
export function reappearLayoutEffects(finishedRoot, current, finishedWork,
...
export function reconnectPassiveEffects(finishedRoot, finishedWork, committedLanes, committedTransitions,
// This function visits both newly finished work and nodes that were re-used
// from a previously committed tree. We cannot check non-static flags if the
// node was reused.
includeWorkInProgressEffects) {
...
export { commitPlacement, commitAttachRef, commitDetachRef, invokeLayoutEffectMountInDEV, invokeLayoutEffectUnmountInDEV, invokePassiveEffectMountInDEV, invokePassiveEffectUnmountInDEV };