...
import { NoLane, SyncLane, OffscreenLane, NoLanes, isSubsetOfLanes, includesBlockingLane, includesOnlyNonUrgentLanes, claimNextTransitionLane, mergeLanes, removeLanes, intersectLanes, isTransitionLane, markRootEntangled, markRootMutableRead, NoTimestamp } from './ReactFiberLane.new';
...
import { getWorkInProgressRoot, getWorkInProgressRootRenderLanes, scheduleUpdateOnFiber, requestUpdateLane, requestEventTime, markSkippedUpdateLanes, isInvalidExecutionContextForEventFunction, getSuspendedThenableState } from './ReactFiberWorkLoop.new';
...
import { markStateUpdateScheduled, setIsStrictModeForDevtools } from './ReactFiberDevToolsHook.new';
import { createCache } from './ReactFiberCacheComponent.new';
import { createUpdate as createLegacyQueueUpdate, enqueueUpdate as enqueueLegacyQueueUpdate, entangleTransitions as entangleLegacyQueueTransitions } from './ReactFiberClassUpdateQueue.new';
import { enqueueConcurrentHookUpdate, enqueueConcurrentHookUpdateAndEagerlyBailout, enqueueConcurrentRenderForLane } from './ReactFiberConcurrentUpdates.new';
import { getTreeId } from './ReactFiberTreeContext.new';
import { now } from './Scheduler';
import { prepareThenableState, trackUsedThenable, checkIfUseWrappedInTryCatch } from './ReactFiberThenable.new';
const {
...
export function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderLanes) {
  ...
  if (didScheduleRenderPhaseUpdateDuringThisPass) {
    ...
    // phase updates).
    children = renderWithHooksAgain(workInProgress, Component, props, secondArg, prevThenableState);
  }
  ...
  if (__DEV__) {
    ...
    if (current !== null && (current.flags & StaticMaskEffect) !== (workInProgress.flags & StaticMaskEffect) &&
...
function renderWithHooksAgain(workInProgress, Component, props, secondArg, prevThenableState) {
