...
describe('SyntheticKeyboardEvent', () => {
    ...
    describe('charCode', () => {
      describe('when event is `keypress`', () => {
        describe('when charCode is present in nativeEvent', () => {
          it('when charCode is 0 and keyCode is 13, returns 13', () => {
            let charCode = null;
            const node = ReactDOM.render(<input onKeyPress={e => {
              charCode = e.charCode;
            }} />, container);
          ...
          it('when charCode is smaller than 32 but is not 13, and keyCode is missing, ignores keypress', () => {
            let called = false;
            const node = ReactDOM.render(<input onKeyPress={() => {
              called = true;
            }} />, container);
            node.dispatchEvent(new KeyboardEvent('keypress', {
              charCode: 31,
      ...
      describe('when event is not `keypress`', () => {
        it('returns 0', () => {
          ...
          let charCodeUp = null;
          const node = ReactDOM.render(<input onKeyDown={e => {
  ...
  describe('EventInterface', () => {
    it('is able to `preventDefault` and `stopPropagation`', () => {
      ...
      const div = ReactDOM.render(<div onKeyDown={eventHandler} onKeyUp={eventHandler} onKeyPress={eventHandler} />, container);
