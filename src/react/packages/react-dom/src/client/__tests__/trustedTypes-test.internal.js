...
describe('when Trusted Types are available in global object', () => {
  ...
  describe('dangerouslySetInnerHTML in svg elements in Internet Explorer', () => {
    ...
    beforeEach(() => {
      ...
      Object.defineProperty(HTMLDivElement.prototype, 'innerHTML', innerHTMLDescriptor);
    });
    afterEach(() => {
    ...
    it('should log a warning', () => {
      class Component extends React.Component {
        render() {
          return <svg dangerouslySetInnerHTML={{
            __html: 'unsafe html'
          }} />;
        }
      }
      ...
      }).toErrorDev("Warning: Using 'dangerouslySetInnerHTML' in an svg element with " + 'Trusted Types enabled in an Internet Explorer will cause ' + 'the trusted value to be converted to string. Assigning string ' + "to 'innerHTML' will throw an error if Trusted Types are enforced. " + "You can try to wrap your svg element inside a div and use 'dangerouslySetInnerHTML' " + 'on the enclosing div instead.');
  ...
  it('should warn once when rendering script tag in jsx on client', () => {
    ...
    }).toErrorDev('Warning: Encountered a script tag while rendering React component. ' + 'Scripts inside React components are never executed when rendering ' + 'on the client. Consider using template tag instead ' + '(https://developer.mozilla.org/en-US/docs/Web/HTML/Element/template).\n' + '    in script (at **)');
