...
import { ClassComponent, FunctionComponent, HostComponent, HostResource, HostSingleton, HostText } from 'react-reconciler/src/ReactWorkTags';
...
import { rethrowCaughtError, invokeGuardedCallbackAndCatchFirstError } from 'shared/ReactErrorUtils';
...
function findAllInRenderedFiberTreeInternal(fiber, test) {
  ...
  const ret = [];
  while (true) {
    if (node.tag === HostComponent || node.tag === HostText || node.tag === ClassComponent || node.tag === FunctionComponent || (enableFloat ? node.tag === HostResource : false) || (enableHostSingletons ? node.tag === HostSingleton : false)) {
...
function makeSimulator(eventType) {
  return function (domNode, eventData) {
    ...
    const event = new SyntheticEvent(reactName, fakeNativeEvent.type, targetInst, fakeNativeEvent, domNode);
...
buildSimulators();
export { renderIntoDocument, isElement, isElementOfType, isDOMComponent, isDOMComponentElement, isCompositeComponent, isCompositeComponentWithType, findAllInRenderedTree, scryRenderedDOMComponentsWithClass, findRenderedDOMComponentWithClass, scryRenderedDOMComponentsWithTag, findRenderedDOMComponentWithTag, scryRenderedComponentsWithType, findRenderedComponentWithType, mockComponent, nativeTouchData, Simulate, act };