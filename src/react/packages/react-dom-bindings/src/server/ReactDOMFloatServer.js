...
import { validatePreloadResourceDifference, validateStyleResourceDifference, validateStyleAndHintProps, validateScriptResourceDifference, validateScriptAndHintProps, validateLinkPropsForStyleResource, validateLinkPropsForPreloadResource, validatePreloadArguments, validatePreinitArguments } from '../shared/ReactDOMResourceValidation';
      ...
      case 'style':
        {
          ...
          if (resource) {
            if (__DEV__) {
              const latestProps = stylePropsFromPreinitOptions(href, resource.precedence, options);
...
export function resourcesFromElement(type, props) {
  if (!currentResources) {
          ...
          case 'font':
            {
              ...
              if (resource) {
                ...
                resource = createPreloadResource(resources, href, as, preloadPropsFromRawProps(href, as, props));
                switch (as) {
                  case 'script':
                    {
                      resources.explicitScriptPreloads.add(resource);
...
// Construct a resource from link props.
export function resourcesFromScript(props) {
  if (!currentResources) {
    throw new Error('"currentResources" was expected to exist. This is a bug in React.');
  }
  const resources = currentResources;
