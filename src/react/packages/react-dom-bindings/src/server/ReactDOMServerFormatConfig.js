...
import { checkHtmlStringCoercion, checkCSSPropertyStringCoercion, checkAttributeStringCoercion } from 'shared/CheckStringCoercion';
import { Children } from 'react';
import { enableFilterEmptyStringAttributesDOM, enableCustomElementPropertySupport, enableFloat, enableFizzExternalRuntime } from 'shared/ReactFeatureFlags';
...
import { prepareToRenderResources, finishRenderingResources, resourcesFromElement, resourcesFromLink, resourcesFromScript, ReactDOMServerDispatcher } from './ReactDOMFloatServer';
export { createResources, createBoundaryResources, setCurrentlyRenderingBoundaryResourcesTarget, hoistResources, hoistResourcesToRoot } from './ReactDOMFloatServer';
import { clientRenderBoundary as clientRenderFunction, completeBoundary as completeBoundaryFunction, completeBoundaryWithStyles as styleInsertionFunction, completeSegment as completeSegmentFunction } from './fizz-instruction-set/ReactDOMFizzInstructionSetInlineCodeStrings';
...
export function createResponseState(identifierPrefix, nonce, bootstrapScriptContent, bootstrapScripts, bootstrapModules, externalRuntimeConfig) {
  ...
  if (enableFizzExternalRuntime) {
    if (externalRuntimeConfig !== undefined) {
      const src = typeof externalRuntimeConfig === 'string' ? externalRuntimeConfig : externalRuntimeConfig.src;
      const integrity = typeof externalRuntimeConfig === 'string' ? undefined : externalRuntimeConfig.integrity;
...
export function pushSegmentFinale(target, responseState, lastPushedText, textEmbedded) {
...
function pushBase(target, props, responseState, textEmbedded, noscriptTagInScope) {
  if (enableFloat && !noscriptTagInScope && resourcesFromElement('base', props)) {
    if (textEmbedded) {
...
function pushMeta(target, props, responseState, textEmbedded, noscriptTagInScope) {
  if (enableFloat && !noscriptTagInScope && resourcesFromElement('meta', props)) {
    if (textEmbedded) {
      // This link follows text but we aren't writing a tag. while not as efficient as possible we need
      // to be safe and assume text will follow by inserting a textSeparator
      target.push(textSeparator);
...
function pushTitle(target, props, responseState, insertionMode, noscriptTagInScope) {
  if (__DEV__) {
    ...
    const childForValidation = Array.isArray(children) && children.length < 2 ? children[0] || null : children;
...
function pushStartHead(target, preamble, props, tag, responseState) {
  return pushStartGenericElement(enableFloat ? preamble : target, props, tag, responseState);
}
...
function pushStartCustomElement(target, props, tag, responseState) {
  ...
  for (let propKey in props) {
    if (hasOwnProperty.call(props, propKey)) {
      ...
      if (enableCustomElementPropertySupport && (typeof propValue === 'function' || typeof propValue === 'object')) {
...
export function pushStartInstance(target, preamble, type, props, responseState, formatContext, textEmbedded) {
  if (__DEV__) {
  ...
  switch (type) {
    ...
    case 'title':
      return enableFloat ? pushTitle(target, props, responseState, formatContext.insertionMode, formatContext.noscriptTagInScope) : pushStartTitle(target, props, responseState);
    case 'link':
      return pushLink(target, props, responseState, textEmbedded, formatContext.noscriptTagInScope);
    case 'script':
      return enableFloat ? pushScript(target, props, responseState, textEmbedded, formatContext.noscriptTagInScope) : pushStartGenericElement(target, props, type, responseState);
    case 'meta':
...
export function pushEndInstance(target, postamble, type, props) {
  switch (type) {
...
export function writeStartCompletedSuspenseBoundary(destination, responseState) {
  return writeChunkAndReturn(destination, startCompletedSuspenseBoundary);
}
export function writeStartPendingSuspenseBoundary(destination, responseState, id) {
...
export function writeStartClientRenderedSuspenseBoundary(destination, responseState, errorDigest, errorMesssage, errorComponentStack) {
  let result;
...
export function writeEndClientRenderedSuspenseBoundary(destination, responseState) {
  return writeChunkAndReturn(destination, endSuspenseBoundary);
}
...
export function writeStartSegment(destination, responseState, formatContext, id) {
...
export function writeCompletedSegmentInstruction(destination, responseState, contentSegmentID) {
...
const completeBoundaryWithStylesScript1FullBoth = stringToPrecomputedChunk(completeBoundaryFunction + ';' + styleInsertionFunction + ';$RR("');
const completeBoundaryWithStylesScript1FullPartial = stringToPrecomputedChunk(styleInsertionFunction + ';$RR("');
...
export function writeCompletedBoundaryInstruction(destination, responseState, boundaryID, contentSegmentID, boundaryResources) {
...
export function writeClientRenderBoundaryInstruction(destination, responseState, boundaryID, errorDigest, errorMessage, errorComponentStack) {
  writeChunk(destination, responseState.startInlineScript);
...
const precedencePlaceholderStart = stringToPrecomputedChunk('<style data-precedence="');
const precedencePlaceholderEnd = stringToPrecomputedChunk('"></style>');
export function writeInitialResources(destination, resources, responseState) {
...
export function writeImmediateResources(destination, resources, responseState) {
  function flushLinkResource(resource) {
    if (!resource.flushed) {
      pushLinkImpl(target, resource.props, responseState);
      resource.flushed = true;
...
function writeStyleResourceAttribute(destination, name, value // not null or undefined
) {
